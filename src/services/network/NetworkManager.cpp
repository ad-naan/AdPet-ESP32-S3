...
void AdPetNetworkManager::handleRoot() {
  ...
  page += "<h2>Voice Trigger</h2><label>Max record ms</label><input name='recordMs' type='number' min='1000' max='8000' value='" + String(c.recordMs) + "'>";
  page += "<p class='hint'>Recording stops early after about 700 ms of silence. Boards without PSRAM are capped at 3000 ms.</p>";
...
void AdPetNetworkManager::handleSave() {
  ...
  if (recordMs > 8000) recordMs = 8000;
